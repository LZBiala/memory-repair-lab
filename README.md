...
| Gaming recall has a visible price (the Goodhart demo) | **hook-bloat arm: 10/10 held-out hits at index cost 215 tok vs repair arm 10/10 at 185 tok** | an authored strategy committed and measured beside the honest one | a demonstration of a failure mode, not evidence any real improver behaves this way |
| The loop terminates | **stop rule fired: no remaining repair-visible failures after generation 1** | the generation loop halts in CI and prints its reason in the ledger | plateau on this corpus says nothing about any other corpus |
